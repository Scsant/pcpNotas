...
;
  ...
  const selecionarTodas = () => {
    const todasVinculadas = notas.filter(nf => nf.status === "vinculada").map(nf => nf.id);
    ...
    <div className="min-h-screen bg-gradient-to-r from-[#0070C0] to-[#00B050] text-white p-6">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-lg overflow-auto p-4">
          ...
          />
          <button
            className="mb-3 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded shadow"
              ...
              <tr className="text-center">
                <th className="px-2 py-2">
                  <input
                    type="checkbox"
                    onChange={(e) => {
                      if (e.target.checked) selecionarTodas();
                      else setNotasSelecionadas([]);
                    }}
                  />
                </th>
          ...
          <button
            className="mb-6 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded shadow disabled:opacity-50"
            disabled={
              notasSelecionadas.length === 0 ||
              notas.some(nf => notasSelecionadas.includes(nf.id) && nf.status !== "vinculada")
            }
